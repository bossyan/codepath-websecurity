<?php
  ...
  if(isset($_POST['submit'])) {
    ...
    $agent = db_fetch_assoc(find_agent_by_id($_GET['id']));
    $sender = db_fetch_assoc(find_agent_by_id($current_user['id']));
    $plain_text = $_POST['plain_text'] ?? '';

    $encrypted_text = pkey_encrypt($plain_text, $sender['public_key']);
