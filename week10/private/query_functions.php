<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    // } else if (!preg_match('/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[#?!@$%^&*-]).{12,}$/', $user['password'])) {
    //   $errors[] = "Password must be at least 12 characters long and contain at least one of each: uppercase letter, lowercase letter, and symbol";
    // }

    // if(is_blank($user['confirm_password'])) {
    //   $errors[] = "Password Confirmation cannot be blank.";
    // } else if (!preg_match('/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[#?!@$%^&*-]).{12,}$/', $user['confirm_password'])) {
    //   $errors[] = "Password Confirmation must be at least 12 characters long and contain at least one of each: uppercase letter, lowercase letter, and symbol";
    // }

    if(!has_valid_password_confirmation($user['password'], $user['confirm_password'])) {
  ...
  function insert_user($user) {
    ...
    $sql .= "'" . db_escape($db, $encrypted_password) . "',";
    $sql .= "'" . $created_at . "'";
    $sql .= ");";
    // For INSERT statements, $result is just true/false
    $result = db_query($db, $sql);
  ...
  function update_user($user) {
    ...
    $sql .= "hashed_password='" . db_escape($db, $encrypted_password) . "',";
  ...
  function get_secret($secret) {
    global $db;
    $sql = 'SELECT * FROM secrets WHERE secret = "' . $secret . '"';
